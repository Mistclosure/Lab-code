# ==============================================================================
# 单细胞 WGCNA 分析流程 (恶性细胞子集伪细胞构建与 Signed 共表达网络)
# ==============================================================================
...
# WGCNA 基础参数 (已更改为 Signed 网络)
COR_TYPE <- "pearson"
NETWORK_TYPE <- "signed" 

# 目标模块提取参数
TARGET_COLORS <- c("brown", "blue", "turquoise", "red", "green")
TARGET_MODULES_DIR <- file.path(FILES_DIR, "Target_Modules_Signed_单独列表")
...
qsave(pseudo_expr_matrix, file.path(QS_DIR, 'pseudo_expr_matrix_signed.qs'))
...
# 6. WGCNA 网络构建 (Signed)
# ==============================================================================
cat("正在计算软阈值 (Signed Network)...\n")
powers <- c(c(1:10), seq(from = 12, to = 30, by = 2))
# sft 的 networkType 保持 "signed"
sft <- pickSoftThreshold(dataExpr, powerVector = powers, networkType = NETWORK_TYPE, verbose = 5)

# 6.1 保存软阈值图到 plots 文件夹
png(filename = file.path(PLOTS_DIR, "WGCNA_Signed_SoftThreshold.png"), width = 10, height = 5, units = "in", res = 300)
par(mfrow = c(1, 2))
cex1 <- 0.9
...
# 6.2 一步法构建共表达网络
cat("正在构建 Blockwise Modules (Signed)...\n")
cor <- WGCNA::cor
net <- blockwiseModules(dataExpr, power = power, maxBlockSize = nGenes,
                        networkType = NETWORK_TYPE, # 指定邻接矩阵为 signed
                        TOMType = NETWORK_TYPE,     # 指定 TOM 矩阵为 signed
                        minModuleSize = 10,
                        reassignThreshold = 0, mergeCutHeight = 0.25,
                        numericLabels = TRUE, pamRespectsDendro = FALSE,
                        saveTOMs = TRUE, corType = COR_TYPE,
                        maxPOutliers = maxPOutliers, loadTOMs = TRUE,
                        saveTOMFileBase = file.path(FILES_DIR, "dataExpr_signed_TOM"),
                        verbose = 3)
...
# 6.3 保存模块聚类树到 plots
png(filename = file.path(PLOTS_DIR, "WGCNA_Signed_ModuleColors_Dendro.png"), width = 8, height = 6, units = "in", res = 300)
plotDendroAndColors(net$dendrograms[[1]], moduleColors[net$blockGenes[[1]]],
                    "Module colors (Signed)", dendroLabels = FALSE, hang = 0.03,
                    addGuide = TRUE, guideHang = 0.05)
dev.off()

# 6.4 提取模块特征基因 (MEs) 并保存热图 1 到 plots
MEs <- net$MEs
MEs_col <- MEs
colnames(MEs_col) <- paste0("ME", labels2colors(as.numeric(str_replace_all(colnames(MEs), "ME", ""))))
MEs_col <- orderMEs(MEs_col)

png(filename = file.path(PLOTS_DIR, "WGCNA_Signed_Eigengene_Adjacency1.png"), width = 7, height = 7, units = "in", res = 300)
...
# 保存特征基因热图 2 到 plots
png(filename = file.path(PLOTS_DIR, "WGCNA_Signed_Eigengene_Adjacency2.png"), width = 7, height = 7, units = "in", res = 300)
...
# 7.1 针对 Blue 模块的可视化
which.module <- "blue"
if (paste0("ME", which.module) %in% colnames(MEs)) {
  ME <- MEs[, paste0("ME", which.module)]
  
  png(filename = file.path(PLOTS_DIR, "WGCNA_Signed_Blue_Module_Plot.png"), width = 8, height = 8, units = "in", res = 300)
  ...
  plotMat(t(scale(dataExpr[, moduleColors == which.module])),
          ...
          main = paste(which.module, "module (Signed)"), cex.main = 2)
...
png(filename = file.path(PLOTS_DIR, "WGCNA_Signed_Network_Heatmap.png"), width = 9, height = 9, units = "in", res = 300)
TOMplot(plotTOM, net$dendrograms[[1]], moduleColors[net$blockGenes[[1]]],
        main = "Network heatmap plot (Signed), all genes",
...
write.table(Colors, file = file.path(FILES_DIR, "moduleLabels_signed.txt"), sep = "\t", quote = FALSE, row.names = FALSE)

# ==============================================================================
# 8. 模块与临床特征关联分析 (原代码注释保留，路径稍作更新备用)
# ==============================================================================
# cat("正在分析模块与临床特征的关联...\n")
...
# png(filename = file.path(PLOTS_DIR, "WGCNA_Signed_Module_trait.png"), width = 7, height = 7, units = "in", res = 300)
...
#                main = "Signed Module-trait relationships")
# dev.off()
...
write.csv(gene_module_df, file = file.path(FILES_DIR, "Module_Genes_signed.csv"), row.names = FALSE, quote = FALSE)
cat("模块基因列表已保存至：", file.path(FILES_DIR, "Module_Genes_signed.csv"), "\n")
...
for (color in TARGET_COLORS) {
  ...
  if (length(single_mod_genes) > 0) {
    ...
    # 转换为数据框以保证导出的CSV具有规范的表头（列名为 "Gene"）
...
qsave(wgcna_results_list, file = file.path(QS_DIR, "WGCNA_Final_Results_signed.qs"))
cat("全部核心分析结果已打包保存至：", file.path(QS_DIR, "WGCNA_Final_Results_signed.qs"), "\n")
cat("全部流程运行完毕！\n")