...
OUT_ROOT <- "/mnt/disk1/qiuzerui/downloads/CRC/pseudo_cilia_score_results/GSE132465"
...
plot_box <- function(data, x_col, y_col, title, ylab, out_png, sample_level = FALSE) {
...
run_group_test <- function(sample_summary, group_col, out_file) {
...
setwd(WORK_DIR)
obj <- qread(file.path(WORK_DIR, "qs", "Seurat", "Malignant_RNA_assay.qs"))
...
obj$metastasis <- ifelse(grepl("M0$", tnm_stage), "Primary", "Metastasis")
obj$Stage_merged <- obj@meta.data[["Stage"]]
obj$Stage_merged[obj$Stage_merged %in% c("IIIA", "IIIB", "IIIC")] <- "III"

group_check <- unique(obj@meta.data[, c("orig.ident", "TNM stage", "metastasis", "Stage_merged"), drop = FALSE])
group_check <- group_check[order(group_check$orig.ident), ]
write.csv(group_check, file.path(OUT_ROOT, "GSE132465_group_assignment_after_trimws.csv"), row.names = FALSE)

for (set_name in names(gene_files)) {
  signature_file <- gene_files[[set_name]]
  signature_name <- tools::file_path_sans_ext(basename(signature_file))
  cilia_genes <- read_gene_list(signature_file)
  ...
  sample_stage <- unique(obj@meta.data[, c("orig.ident", "Stage_merged"), drop = FALSE])
  colnames(sample_stage) <- c("sample_id", "Stage")
  result$score <- merge(result$score, sample_stage, by = "sample_id", all.x = TRUE, sort = FALSE)
  result$sample_summary <- merge(result$sample_summary, sample_stage, by = "sample_id", all.x = TRUE, sort = FALSE)

  save_pseudoCiliaScore_results(result, outdir)

  plot_box(result$score, "group", "CiliaScore",
           paste0(signature_name, "+", DATASET, "+pseudoCiliaScore (pseudo-cell metastasis)"),
           "Pseudo-cell CiliaScore",
           file.path(plot_dir, paste0(signature_name, "_pseudo_cell_metastasis.png")))

  plot_box(result$sample_summary, "group", "mean_CiliaScore",
           paste0(signature_name, "+", DATASET, "+pseudoCiliaScore (sample metastasis)"),
           "Sample mean CiliaScore",
           file.path(plot_dir, paste0(signature_name, "_sample_metastasis.png")),
           sample_level = TRUE)

  plot_box(result$sample_summary, "Stage", "mean_CiliaScore",
           paste0(signature_name, "+", DATASET, "+pseudoCiliaScore (sample stage)"),
           "Sample mean CiliaScore",
           file.path(plot_dir, paste0(signature_name, "_sample_stage.png")),
           sample_level = TRUE)

  p_scatter <- ggplot(result$score, aes(x = background_score, y = target_score, color = group)) +
  ...
  run_group_test(result$sample_summary, "group", file.path(outdir, "sample_metastasis_wilcox.txt"))
}
