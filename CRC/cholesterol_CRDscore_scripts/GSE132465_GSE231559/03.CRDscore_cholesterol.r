...
PLOTS_DIR <- "/mnt/disk1/qiuzerui/downloads/CRC/cholesterol_Score/GSE132465_GSE231559"
...
p1 <- ggplot(data_stage, aes(x = Type, y = score, color = Type)) +
  ...
  ggtitle(paste0(signature_name, "+GSE132465_GSE231559+CRDscore (Stage)")) +
...
print(p1)
ggsave(file.path(PLOTS_DIR, paste0(signature_file_prefix, "_GSE132465_GSE231559_Stage_plot.png")), plot = p1, width = 6, height = 5, dpi = 300)
...
p2 <- ggplot(data_meta, aes(x = Type, y = score, color = Type)) +
  ...
  theme(legend.position = "none") +
  ggtitle(paste0(signature_name, "+GSE132465_GSE231559+CRDscore (Metastasis)")) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  ylab("CRDScore") +
  theme(axis.title.x = element_blank(),
        axis.text.y = element_text(size = 15, face = "bold", color = "black"),
...
print(p2)
ggsave(file.path(PLOTS_DIR, paste0(signature_file_prefix, "_GSE132465_GSE231559_Metastasis_plot.png")), plot = p2, width = 6, height = 5, dpi = 300)
