...
# --- 配置 ---
WORK_DIR <- '/mnt/disk1/qiuzerui/downloads/CRC/cillascore/GSE178341'
...
# --- 读取 Seurat 对象 ---
pbmc1 <- qread('/mnt/disk1/qiuzerui/downloads/CRC/GSE178341/Malignant.qs')
...
cli <- read.csv('/mnt/disk1/qiuzerui/downloads/CRC/GSE178341/Cli.csv', header = TRUE, check.names = FALSE)
rt <- merge(meta, cli, by.x = "orig.ident", by.y = "PatientBarcode")

# --- 自动检测 Primary/Metastasis 分组 ---
detected <- FALSE
for (col in c("metastasis", "Metastasis", "type", "Type", "Tumor Stage", "Stage")) {
  if (col %in% colnames(rt)) {
    vals <- as.character(rt[[col]])
    if (any(grepl("metasta|Metasta|LM", vals, ignore.case = TRUE))) {
      rt$group <- ifelse(grepl("metasta|Metasta|LM", vals, ignore.case = TRUE), "Metastasis", "Primary")
      detected <- TRUE
      cat(sprintf("Auto-detected metastasis from column '%s'\n", col))
      break
    }
  }
}
if (!detected) stop("Cannot detect Primary/Metastasis column in GSE178341 metadata.")
