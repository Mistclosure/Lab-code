...
# 1. 修改：循环读取并自动重命名文件为标准 10X 格式
samples <- c("C1", "C2", "C3", "C4", "L1", "L2")
scRNA_list <- list()

for (i in samples) {
  # --- 新增：自动重命名代码块 ---
  # 获取当前文件夹下的所有文件路径
  all_files <- list.files(i, full.names = TRUE)
  
  # 定义匹配逻辑并重命名
  # 匹配 barcodes
  b_file <- all_files[grep("barcodes.tsv.gz$", all_files)]
  if(length(b_file) > 0) file.rename(b_file, file.path(i, "barcodes.tsv.gz"))
  
  # 匹配 features 或 genes
  f_file <- all_files[grep("features.tsv.gz$|genes.tsv.gz$", all_files)]
  if(length(f_file) > 0) file.rename(f_file, file.path(i, "features.tsv.gz"))
  
  # 匹配 matrix
  m_file <- all_files[grep("matrix.mtx.gz$", all_files)]
  if(length(m_file) > 0) file.rename(m_file, file.path(i, "matrix.mtx.gz"))
  # ----------------------------

  # 读取已经变为标准格式的数据
  tmp_data <- Read10X(i) 
  scRNA_list[[i]] <- CreateSeuratObject(tmp_data, project = i, min.cells = 3, min.features = 300)
}

# 合并所有样本为一个对象
scRNA <- merge(scRNA_list[[1]], y = scRNA_list[-1], add.cell.ids = samples)
...
# ==================== 按 orig.ident (C1-L2) 分批运行 CopyKAT ====================
