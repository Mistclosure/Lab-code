...
rloop_data = read.csv("112 primary cilium genes.csv", header = T, check.names = F)
#target_genes =c('CXCL9', 'CXCL10', 'CXCL11', 'CXCR3', 'CD3', 'CD4', 'CD8a','CD8', 'CD8b', 'CD274', 'PDCD1', 'CXCR4', 'CCL5')
# 提取 GeneSymbol 列的前 92 个基因
target_genes = as.character(rloop_data$'Gene Symbol'[1:112])
...
data = rt1[,c('score','Stage')]
colnames(data) = c("score","Type")
data$Type[data$Type=='Stage II']='Stage I'
...
ggplot(data,aes(x = Type , y=score, color=Type))+
  ...
  ggtitle("LUAD+primary cilium 112 genes") +
  ylim(-0.4, 0.4) +
