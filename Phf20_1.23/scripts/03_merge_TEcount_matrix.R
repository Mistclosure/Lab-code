...
metadata_file <- file.path(files_dir, "Phf20_1.23_edgeR_sample_metadata.csv")
...
infer_metadata <- function(count_dir) {
  ...
  if (length(files) == 0) {
    stop("Metadata not found and no .cntTable files available for inference: ", count_dir)
  }
  ...
  if (any(is.na(group))) {
    stop("Cannot infer group for samples: ", paste(sample[is.na(group)], collapse = ", "),
         "\nPlease create metadata manually at: ", metadata_file)
  }
  metadata <- data.frame(sample = sample, group = group, stringsAsFactors = FALSE)
  metadata <- metadata[order(factor(metadata$group, levels = c("Scr", "Phf20")), metadata$sample), ]
  write.csv(metadata, metadata_file, row.names = FALSE)
  message("Metadata was inferred from count files and written to: ", metadata_file)
  metadata
}
