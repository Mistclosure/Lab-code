...
# SingleR全局初步注释 -> 绘制全细胞UMAP -> 提取髓系重聚类 -> 基于差异基因人工校验注释 -> 
# 分组绘图对比 -> Fkbp5+ 单核细胞靶向分析
# ==============================================================================
...
# ------------------------------------------------------------------------------
# 6. 绘制所有细胞的聚类图片 (分组织、分温度)
# ------------------------------------------------------------------------------
print("🚀 步骤5/10: 绘制所有细胞的聚类 UMAP (Total + 各温度组)...")

for (tissue_name in names(sc_by_tissue)) {
  print(paste(">>> 正在绘制组织:", tissue_name, "的所有细胞聚类图..."))
  obj <- sc_by_tissue[[tissue_name]]
  
  p_total <- DimPlot(obj, reduction = "umap", group.by = "seurat_clusters", label = TRUE, repel = TRUE) + 
    ggtitle(paste(tissue_name, "- All Cells")) +
    theme(plot.title = element_text(hjust = 0.5, size = 14, face = "bold"),
          axis.title = element_blank())
  
  p_cold <- DimPlot(subset(obj, subset = Group == "Cold_4C"), reduction = "umap", group.by = "seurat_clusters", label = FALSE) + 
    ggtitle("Cold_4C") + theme(plot.title = element_text(hjust = 0.5, size = 12), axis.title = element_blank())
  
  p_rt <- DimPlot(subset(obj, subset = Group == "RT_25C"), reduction = "umap", group.by = "seurat_clusters", label = FALSE) + 
    ggtitle("RT_25C") + theme(plot.title = element_text(hjust = 0.5, size = 12), axis.title = element_blank())
  
  p_tn <- DimPlot(subset(obj, subset = Group == "TN_30C"), reduction = "umap", group.by = "seurat_clusters", label = FALSE) + 
...
# ------------------------------------------------------------------------------
# 8. 独立循环导出每个组织每个 Cluster 的 Top 20 Markers
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# 9. 差异表达分析 (Markers) 与人工校对注释逻辑
# ------------------------------------------------------------------------------
print("🚀 步骤8/10: 运行 FindAllMarkers 并注释髓系亚群...")
...
# ------------------------------------------------------------------------------
# 11. Fkbp5+ / Fkbp5- 单核细胞专属分析 (VlnPlot & UMAP FeaturePlot)
# ------------------------------------------------------------------------------
...
for (tissue_name in names(sc_myeloid_list)) {
  ...
  # 判断是否有 Monocytes
  if (!"Monocytes" %in% unique(obj$Myeloid_Subtype)) {
    print(paste("   ⚠️", tissue_name, "中未检测到 Monocytes，跳过该组织。"))
    next
  }
  
  # 提取 Monocytes
  mono_obj <- subset(obj, subset = Myeloid_Subtype == "Monocytes")
  ...
  # --- (2) UMAP FeaturePlot (参考附图，2x2 布局) ---
  # 使用 scale_color_gradientn 匹配附图从浅灰到红色的映射
