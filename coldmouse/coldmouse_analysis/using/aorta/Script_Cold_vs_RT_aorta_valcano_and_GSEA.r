# ==============================================================================
# Script: Whole Aorta Cold vs RT 差异分析 + MSigDB Mouse GSEA
# 重点输出图片
# ==============================================================================
...
output_prefix <- paste(tissue_name, analysis_scope, "Cold_vs_RT", sep = "_")
...
print(paste0("📌 当前比较: ", group_cold, " vs ", group_rt))
...
# 过滤 TN_30C，只保留 Cold_4C 和 RT_25C
aorta_obj <- subset(aorta_obj, subset = Group %in% c(group_cold, group_rt))

Idents(aorta_obj) <- "Group"

missing_groups <- setdiff(c(group_cold, group_rt), levels(Idents(aorta_obj)))
...
print("🚀 步骤2: 计算 Whole Aorta Cold vs RT 差异基因...")
...
p_volcano <- ggplot(
  ...
  labs(
    title = paste0("Volcano: ", tissue_name, " ", analysis_scope, " ", group_cold, " vs ", group_rt),
...
top_gsea_bar <- bind_rows(
  ...
  mutate(
    Direction = ifelse(NES > 0, "Enriched in Cold_4C", "Enriched in RT_25C"),
...
if (nrow(top_rt_term) == 1) {
  ...
  p_top_rt <- gseaplot2(
    ...
    title = paste0("Top RT-enriched: ", top_rt_term$Description[1])
  )
  
  ggsave(
    filename = file.path("plots", paste0("GSEA_Top_RT_", output_prefix, ".png")),
    plot = p_top_rt,
    width = 8,
    height = 6,
    dpi = 300
  ...
  ggsave(
    filename = file.path("plots", paste0("GSEA_Top_RT_", output_prefix, ".pdf")),
    plot = p_top_rt,
    width = 8,
    height = 6
  )
...
print(paste0("  plots/GSEA_Top_RT_", output_prefix, ".png"))