# ==============================================================================
# Script: Monocytes 整体 Cold vs TN 差异基因分析、火山图及 GO 富集
# ==============================================================================
...
mono_cells <- subset(mono_cells, subset = Group == 'RT_25C', invert = TRUE)

# 【关键点】将当前鉴定的标准切换为 Group，以便比较 Cold 和 TN
Idents(mono_cells) <- "Group"

# 注意：请确保您的 metadata 中包含确切的 "Cold" 和 "TN" 标签
# 如果您的命名是 "Cold_4C" 或 "TN_25C"，请在下方 ident.1 和 ident.2 中对应修改！
group_cold <- "Cold_4C" 
group_tn   <- "TN_30C"

# ------------------------------------------------------------------------------
# 2. 计算 Cold vs TN 的所有差异基因
# ------------------------------------------------------------------------------
print(paste0("🚀 步骤2: 正在计算 ", group_cold, " vs ", group_tn, " 的差异基因..."))
...
# 保存差异分析全表
#write.csv(deg_results, file = file.path("files", "DEG_Monocytes_Cold_vs_TN.csv"), row.names = FALSE)
...
volcano_plot <- ggplot(deg_results, aes(x = avg_log2FC, y = -log10(p_val_adj), color = Significance)) +
  ...
  labs(title = paste0("Volcano Plot: ", group_cold, " vs ", group_tn, " in Monocytes"),
       x = expression("Log"[2]*" Fold Change"),
       y = expression("-Log"[10]*" (Adjusted P-value)")) +
  theme(legend.position = "top", legend.title = element_blank(), plot.title = element_text(hjust = 0.5))

# 保存火山图
ggsave(filename = file.path("plots", "Volcano_Monocytes_Cold_vs_TN.pdf"), plot = volcano_plot, width = 8, height = 7)
ggsave(filename = file.path("plots", "Volcano_Monocytes_Cold_vs_TN.png"), plot = volcano_plot, width = 8, height = 7, dpi = 300)
...
if (nrow(all_go_results) > 0) {
  ...
  output_go_file <- file.path("files", "GO_BP_Enrichment_Monocytes_Cold_vs_TN.csv")
