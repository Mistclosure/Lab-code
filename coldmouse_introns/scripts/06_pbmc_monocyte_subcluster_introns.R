...
mono_cells <- FindClusters(mono_cells, resolution = 0.35, algorithm = 1)  # Louvain
...
# 生成 mono_cluster_id
mono_cells$mono_cluster_id <- paste0("Mono_", as.numeric(as.character(mono_cells$seurat_clusters)) + 1)
cluster_numbers <- sort(unique(as.numeric(as.character(mono_cells$seurat_clusters))))
mono_cells$mono_cluster_id <- factor(mono_cells$mono_cluster_id,
                                     levels = paste0("Mono_", seq_along(cluster_numbers)))
...
write.csv(all_markers,
          file.path(files_dir, "introns_Markers_All_Monocytes_Subgroups.csv"),
          row.names = FALSE)
write.csv(top30_markers,
          file.path(files_dir, "introns_Markers_Top30_Monocytes_Subgroups.csv"),
          row.names = FALSE)
cat("Marker 表已保存\n")
...
ggsave(file.path(plots_dir, "introns_UMAP_Monocytes_Subgroups_Local.png"),
       plot = p_final_local, width = 15, height = 11, dpi = 300)
ggsave(file.path(plots_dir, "introns_UMAP_Monocytes_Subgroups_Local.pdf"),
...
ggsave(file.path(plots_dir, "introns_UMAP_Monocytes_Subgroups_Global.png"),
       plot = p_final_cls, width = 15, height = 11, dpi = 300)

# 保存 monocyte 子集对象
qsave(mono_cells, file.path(objects_dir, "coldmouse_introns_pbmc_monocytes_subclustered.qs"))
...
write.csv(deg_cold_vs_rt,
          file.path(files_dir, "introns_DEG_Monocytes_Cold_vs_RT.csv"),
          row.names = FALSE)
...
write.csv(deg_cold_vs_tn,
          file.path(files_dir, "introns_DEG_Monocytes_Cold_vs_TN.csv"),
          row.names = FALSE)
...
for (cl in levels(mono_cells$mono_cluster_id)) {
  ...
  write.csv(deg_tmp,
            file.path(files_dir, paste0("introns_DEG_", cl, "_vs_Others.csv")),
            row.names = FALSE)

  top20 <- deg_tmp %>% slice_max(n = 20, order_by = avg_log2FC, with_ties = FALSE)
  write.csv(top20,
            file.path(files_dir, paste0("introns_DEG_", cl, "_vs_Others_Top20.csv")),
            row.names = FALSE)
}

cat("\n=== PBMC Monocyte 亚群分析完成 ===\n")
cat("对象保存:", file.path(objects_dir, "coldmouse_introns_pbmc_monocytes_subclustered.qs"), "\n")
cat("结果目录:", files_dir, "\n")
cat("图片目录:", plots_dir, "\n")
