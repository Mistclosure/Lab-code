# ==============================================================================
# 02_clustering_and_global_anno_no_harmony_introns.R
# coldmouse_introns：无 Harmony 的降维聚类、SingleR 全局注释和 marker 输出
# ==============================================================================
# 输入：coldmouse_introns_sc_combined_qc_Cleaned.qs
# 输出：coldmouse_introns_sc_by_tissue_no_harmony_louvain.qs、marker csv、UMAP png
# 参考：docs/original_reference/clustering_and_global_anno_no_harmony.r
# 主要修改：
#   - 路径和参数从 config.yaml 获取
#   - 输出文件名包含 coldmouse_introns/introns 标识
#   - 不运行 Harmony
# ==============================================================================
...
sc_by_tissue <- lapply(names(sc_by_tissue), function(x) {
  run_seurat_standard(sc_by_tissue[[x]], x,
                      method_id = algo_id,
                      n_pcs = n_pcs,
...
save_seurat(sc_by_tissue,
            file.path(objects_dir,
                      paste0("coldmouse_introns_sc_by_tissue_no_harmony_", cluster_method, "_with_markers_input.qs")))
print("步骤4/4：脚本结束。请在 results/ 中查看 introns 版本输出。")
